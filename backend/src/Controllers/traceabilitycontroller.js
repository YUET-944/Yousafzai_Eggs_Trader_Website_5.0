...
export const getTraceability = async (req, res) => {
  ...
  try {
    // Find the pipeline record for this brandId
    let pipeline = await Traceability.findOne({ where: { brandId } });
    ...
    // Convert Sequelize instance to plain JSON object so we can safely manipulate it
    ...
    if (Array.isArray(pipelineData.stages)) {
      pipelineData.stages.sort((a, b) => a.order - b.order);
    }
...
export const updateTraceability = async (req, res) => {
  ...
  try {
    ...
    // Find existing pipeline or create a new one
    let pipeline = await Traceability.findOne({ where: { brandId } });

    if (!pipeline) {
      pipeline = await Traceability.create({ brandId, stages });
    } else {
